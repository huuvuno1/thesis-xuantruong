import React from 'react';
import './index.css';

export default function CartPaymentResponsive() {
  return (
    <div className="main-container">
      <div className="rectangle">
        <div className="ra-mat-simple-back-to-school">
          <span className="ra-mat-simple-back-to-school-1">
            Ra mắt 365 Simple Back to school!
          </span>
          <span className="mua-ngay">Mua ngay</span>
          <span className="ra-mat-simple-back-to-school-2">{' ->'}</span>
        </div>
        <div className="rectangle-3">
          <div className="asset" />
          <div className="align-left">
            <div className="vector" />
            <div className="vector-4" />
            <div className="vector-5" />
          </div>
          <div className="account">
            <div className="vector-6" />
          </div>
          <div className="cart">
            <div className="vector-7" />
            <div className="vector-8" />
          </div>
          <div className="search" />
          <div className="ellipse" />
          <span className="number">5</span>
        </div>
      </div>
      <div className="scroll">
        <div className="gio-hang-thanh-toan-hoan-tat">
          <span className="hoan-tat">Hoàn tất</span>
          <span className="thanh-toan">Thanh toán</span>
          <span className="gio-hang">Giỏ hàng</span>
          <div className="vector-9" />
          <div className="vector-a" />
        </div>
        <div className="group">
          <div className="ho-ten">
            <span className="ho-va-ten">Họ và tên</span>
            <div className="rectangle-b" />
            <input className="ho-ten-input" />
          </div>
          <div className="flex-row">
            <button className="email">
              <span className="email-c">Email</span>
              <div className="rectangle-d" />
            </button>
            <button className="sdt">
              <span className="so-dien-thoai">Số điện thoại</span>
              <div className="rectangle-e" />
            </button>
          </div>
          <div className="flex-row-f">
            <button className="email-10">
              <div className="rectangle-11" />
              <span className="thanh-pho">Thành phố</span>
              <div className="formkit-down">
                <div className="vector-12" />
              </div>
            </button>
            <button className="sdt-13">
              <div className="rectangle-14" />
              <span className="quan-huyen">Quận/Huyện</span>
              <div className="formkit-down-15">
                <div className="vector-16" />
              </div>
            </button>
          </div>
          <div className="flex-row-ea">
            <div className="email-17">
              <div className="rectangle-18" />
              <span className="xa-phuong">Xã/phường</span>
              <div className="formkit-down-19">
                <div className="vector-1a" />
              </div>
            </div>
            <button className="button-sdt">
              <span className="dia-chi">Địa chỉ</span>
              <div className="rectangle-1b" />
            </button>
          </div>
          <div className="ghi-chu-them">
            <span className="ghi-chu-them-1c">Ghi chú thêm</span>
            <div className="rectangle-1d" />
          </div>
        </div>
        <div className="gia">
          <div className="gia-1e">
            <div className="flex-row-eaa">
              <div className="group-1f">
                <span className="nhap-ma-giam-gia">Nhập mã giảm giá</span>
                <div className="rectangle-20" />
              </div>
              <div className="rectangle-21" />
              <span className="ap-dung">Áp dụng</span>
            </div>
            <div className="flex-row-22">
              <span className="tam-tinh">Tạm tính</span>
              <span className="comma">1,647,000</span>
            </div>
            <div className="flex-row-c">
              <span className="giam-gia">Giảm giá</span>
              <span className="empty">0</span>
            </div>
            <div className="flex-row-be">
              <span className="phi-giao-hang">Phí giao hàng</span>
              <span className="mien-phi">Miễn phí</span>
            </div>
            <div className="flex-row-23">
              <span className="tong">Tổng</span>
              <span className="comma-24">1,647,000</span>
            </div>
          </div>
        </div>
        <div className="phg-thuc">
          <span className="phuong-thuc-thanh-toan">Phương thức thanh toán</span>
          <div className="chon-hinh-thuc">
            <div className="flex-row-a">
              <div className="rectangle-25" />
              <div className="vector-26" />
              <span className="cod-thanh-toan-khi-nhan-hang">
                COD (Thanh toán khi nhận hàng)
              </span>
            </div>
            <div className="flex-row-fcc">
              <div className="rectangle-27" />
              <div className="vector-28" />
              <span className="thanh-toan-qua-vi-momo">
                Thanh toán qua ví MoMo
              </span>
            </div>
            <div className="flex-row-fb">
              <div className="rectangle-29" />
              <div className="vector-2a" />
              <span className="thanh-toan-qua-vi-dien-tu">
                Thanh toán qua ví điện tử (ZaloPay / VNPAY..)
              </span>
            </div>
            <div className="flex-row-d">
              <div className="rectangle-2b" />
              <div className="vector-2c" />
              <span className="quet-qr-thanh-toan-bang-ung-dung-ngan-hang">
                Quét QR / Thanh toán bằng ứng dụng ngân hàng
              </span>
            </div>
          </div>
        </div>
        <div className="button-tien-hanh-dat-hang">
          <span className="hoan-tat-don-hang">Hoàn tất đơn hàng</span>
          <div className="rectangle-2d" />
        </div>
        <div className="footer">
          <div className="footer-2e">
            <div className="group-2f">
              <div className="simple-chon-don-gian-song-de-dang">
                <span className="text-21">365 Simple </span>
                <span className="chon-don-gian-song-de-dang">
                  - Chọn đơn giản - Sống dễ dàng !
                </span>
              </div>
              <span className="chung-toi-luon-tran-trong">
                Chúng tôi luôn trân trọng và mong đợi nhận được mọi ý kiến đóng
                góp từ khách hàng để có thể nâng cấp trải nghiệm dịch vụ và sản
                phẩm tốt hơn nữa.
              </span>
            </div>
            <div className="button-dong-gop">
              <span className="dong-gop-y-kien">Đóng góp ý kiến</span>
              <div className="rectangle-30" />
            </div>
            <div className="hotline">
              <div className="flex-column-b">
                <span className="hotline-31">Hotline</span>
                <span className="text-26">1900-100023 - 027.918.2981</span>
              </div>
              <div className="ph-phone-light">
                <div className="vector-32" />
              </div>
            </div>
            <div className="email-33">
              <div className="flex-column-bd">
                <span className="email-34">Email</span>
                <span className="text-28">365simple@gmail.com</span>
              </div>
              <div className="material-symbols-mail-outline">
                <div className="vector-35" />
              </div>
            </div>
            <div className="lien-he">
              <div className="ig" />
              <div className="youtube" />
              <div className="whatsapp" />
              <div className="facebook" />
            </div>
            <div className="rectangle-36" />
            <div className="footer-37">
              <div className="ndung-footer">
                <span className="dia-chi-lien-he">Địa chỉ liên hệ</span>
                <div className="line" />
                <div className="club">
                  <span className="text-2a">365 Club</span>
                  <span className="dang-ky-thanh-vien">Đăng ký thành viên</span>
                  <span className="uu-dai-doc-quyen">Ưu đãi & độc quyền</span>
                </div>
                <div className="chinh-sach">
                  <span className="chinh-sach-38">Chính sách</span>
                  <span className="dieu-khoan-va-dieu-kien">
                    Điều khoản và điều kiện
                  </span>
                  <span className="chinh-sach-khuyen-mai">
                    Chính sách khuyến mãi
                  </span>
                  <span className="chinh-sach-bao-mat">Chính sách bảo mật</span>
                  <span className="chinh-sach-giao-hang">
                    Chính sách giao hàng
                  </span>
                </div>
                <div className="cham-soc-khach-hang">
                  <span className="cham-soc-khach-hang-39">
                    Chăm sóc khách hàng
                  </span>
                  <span className="trai-nghiem-mua-sam-hai-long">
                    Trải nghiệm mua sắm 100% hài lòng
                  </span>
                  <span className="hoi-dap-faqs">Hỏi đáp - FAQs</span>
                </div>
                <div className="kien-thuc-mac-dep">
                  <span className="kien-thuc-mac-dep-3a">
                    Kiến thức mặc đẹp
                  </span>
                  <span className="huong-dan-chon-size">
                    Hướng dẫn chọn size
                  </span>
                  <span className="blog">Blog</span>
                  <span className="group-mac-don-gian-song-chat">
                    Group mặc đơn giản sống chất
                  </span>
                </div>
              </div>
              <div className="group-3b">
                <div className="line-3c" />
                <span className="giay-phep-kinh-doanh-lu-hanh-quoc-te-so">
                  Giấy phép kinh doanh lữ hành quốc tế số: 01-1761/2023/TCDL-GP
                  LHQT
                </span>
                <div className="image" />
                <span className="copyright">
                  Copyright © 365 Simple Viet Nam
                </span>
              </div>
            </div>
            <div className="co-so">
              <span className="address">
                Cơ sở 1: Số XXX Nguyễn Văn A, phường Cống Vị, quận Ba Đình,
                Thành phố Hà Nội
              </span>
              <div className="ic-baseline-place">
                <div className="vector-3d" />
              </div>
            </div>
            <div className="co-so-3e">
              <div className="ic-baseline-place-3f">
                <div className="vector-40" />
              </div>
              <span className="address-41">
                Cơ sở 2: Số XXX Nguyễn Văn A, phường Cống Vị, quận Ba Đình,
                Thành phố Hà Nội
              </span>
            </div>
          </div>
        </div>
        <span className="shipping-info">Thông tin vận chuyển</span>
        <div className="o-tich">
          <div className="rectangle-42" />
        </div>
        <span className="default-address">Sử dụng địa chỉ mặc định</span>
        <div className="cart-43">
          <span className="cart-44">Giỏ hàng</span>
          <div className="product">
            <div className="product-name">
              <span className="product-name-45">Áo khoác lông S2</span>
              <div className="rectangle-46" />
              <span className="price">1.799.000</span>
              <button className="product-size">
                <div className="group-47">
                  <span className="size">S</span>
                  <div className="vector-48" />
                </div>
                <div className="rectangle-49" />
              </button>
              <button className="product-color">
                <div className="group-4a">
                  <span className="color">Be</span>
                  <div className="vector-4b" />
                </div>
                <div className="rectangle-4c" />
              </button>
              <div className="quantity" />
              <div className="vector-4d" />
              <button className="add-quantity">
                <div className="group-4e" />
                <div className="group-4f">
                  <div className="rectangle-50" />
                  <div className="add-icon">
                    <div className="vector-51" />
                  </div>
                </div>
                <div className="rectangle-52" />
                <span className="quantity-number">1</span>
              </button>
            </div>
            <div className="product-53">
              <div className="flex-row-54">
                <div className="rectangle-55" />
                <span className="product-name-56">Quần tây nam SiteTab v2</span>
                <span className="price-57">1.799.000</span>
              </div>
              <div className="flex-row-58">
                <button className="product-color-59">
                  <div className="group-5a">
                    <span className="color-5b">Đen</span>
                    <div className="vector-5c" />
                  </div>
                  <div className="rectangle-5d" />
                </button>
                <button className="product-size-5e">
                  <div className="group-5f">
                    <span className="s">S</span>
                    <div className="vector-60" />
                  </div>
                  <div className="rectangle-61" />
                </button>
                <div className="ph-x" />
              </div>
              <div className="vector-62" />
              <div className="them-so-luong">
                <div className="group-63" />
                <div className="group-64">
                  <div className="rectangle-65" />
                  <div className="mdi-add-bold">
                    <div className="vector-66" />
                  </div>
                </div>
                <div className="rectangle-67" />
                <span className="text-49">1</span>
              </div>
            </div>
            <div className="san-pham">
              <span className="quan-jeans-slimfit">Quần Jeans Slimfit</span>
              <div className="rectangle-68" />
              <span className="text-4b">1.799.000</span>
              <div className="kich-thuoc-san-pham">
                <div className="group-69">
                  <span className="s-6a">S</span>
                  <div className="vector-6b" />
                </div>
                <div className="rectangle-6c" />
              </div>
              <div className="mau-sac-gio-hang">
                <div className="group-6d">
                  <span className="den">Đen</span>
                  <div className="vector-6e" />
                </div>
                <div className="rectangle-6f" />
              </div>
              <div className="ph-x-70" />
              <div className="vector-71" />
              <div className="them-so-luong-72">
                <div className="group-73" />
                <div className="group-74">
                  <div className="rectangle-75" />
                  <div className="mdi-add-bold-76">
                    <div className="vector-77" />
                  </div>
                </div>
                <div className="rectangle-78" />
                <span className="text-4e">1</span>
              </div>
            </div>
          </div>
        </div>
        <span className="tien-hanh-dat-hang">Tiến hành đặt hàng</span>
      </div>
      <div className="rectangle-79">
        <div className="rectangle-7a">
          <div className="home" />
        </div>
        <div className="rectangle-7b">
          <div className="archive" />
        </div>
        <div className="rectangle-7c">
          <div className="clock" />
        </div>
        <div className="rectangle-7d">
          <div className="heart">
            <div className="vector-7e" />
          </div>
        </div>
        <div className="rectangle-7f">
          <div className="phone">
            <div className="vector-80" />
          </div>
        </div>
      </div>
      <div className="home-indicator-on-light">
        <div className="rectangle-81" />
      </div>
    </div>
  );
}
